...
with tabs[9]:
    ...
    with col_info:
        ...
        # --- Keep the Terrain selector synced to the last rolled terrain ---
        # The Terrain dropdown represents the *previous* hex terrain you are leaving.
        # After a roll, we force the dropdown to the newly rolled terrain so the next hex roll is automatic.
        terrain_key = f"map_terrain_{selected_hex}"
        _force = st.session_state.get("_map_force_terrain")
        if isinstance(_force, dict) and _force.get("hex") == selected_hex:
            st.session_state.pop("_map_force_terrain", None)
            st.session_state[terrain_key] = (_force.get("value") or st.session_state.get("map_default_terrain") or "Landing")
            # Store the actual terrain on the hex for later reference (hover, summaries, etc.)
            try:
                hex_map[selected_hex]["terrain"] = st.session_state[terrain_key]
            except Exception:
                pass
        else:
            # For unrolled hexes, always sync the selector to the last rolled terrain (except initial Landing).
            if not d.get("td_short"):
                st.session_state[terrain_key] = (st.session_state.get("map_default_terrain") or "Landing")
        ...
        if st.button("Roll Hex", key=f"btn_roll_hex_{selected_hex}"):
            ...
            hex_map[selected_hex]["terrain"] = final_terrain or td_head
            if final_terrain:
            ...
            st.session_state["_map_show_roll_success"] = True
            st.session_state["_map_force_terrain"] = {"hex": selected_hex, "value": st.session_state.get("map_default_terrain", "Landing")}
            st.rerun()

        # Show roll confirmation after the forced rerun (so we can safely update widget values).
        if st.session_state.pop("_map_show_roll_success", False):

...
def format_row_for_display(table_name: str, row: pd.Series) -> str:
    ...
    # --- Special case: Known Threats (render ALL columns from known_threat.csv) ---
    if table_name == "known_threat":
        def fmt(v):
            if v is None:
                return ""
            # pandas NaN
        ...
        def parse_recovery_reactions(raw) -> list[str]:
            s = fmt(raw)
            if not s:
                return []
            ...
            for seg in segs:
                ...
                if m:
                    status = m.group(1).strip()
                    rest = m.group(2).strip()
                    out.append(f"- **[{status}]** {rest}" if rest else f"- **[{status}]**")
                else:
                    out.append(f"- {seg}")
            return out

        name = fmt(row.get("name", "")) or "Unknown Threat"
        creature_type = fmt(row.get("creature_type", ""))
        size = fmt(row.get("size", ""))
        movement = fmt(row.get("movement", ""))
        atk_skill = fmt_signed(row.get("attack_skill", ""))
        atk_range = fmt(row.get("attack_range", ""))

        # Attack range: numeric => assume hexes
        atk_range_disp = ""
        if atk_range:
            if re.fullmatch(r"[+-]?\d+(?:\.\d+)?", atk_range):
                atk_range_disp = f"{atk_range} hexes"
            else:
                atk_range_disp = atk_range

        target_priority = fmt(row.get("target_priority", ""))
        recovery_reactions = parse_recovery_reactions(row.get("recovery_reactions", ""))
        unique_behavior = fmt(row.get("unique_behavior", ""))
        special_effect = fmt(row.get("special_effect", ""))

        lines: list[str] = [name, ""]

        if creature_type:
            lines.append(f"**Creature Type:** {creature_type}  " )
        if size:
        ...
        # Attack block
        if atk_skill or atk_range_disp:
            if atk_range_disp.lower() == "melee":
                lines.append(f"- **Attack:** Attack Skill {atk_skill or ''}, Melee")
            elif atk_range_disp:
                lines.append(f"- **Attack:** Attack Skill {atk_skill or ''}, Range {atk_range_disp}")
            else:
                lines.append(f"- **Attack:** Attack Skill {atk_skill or ''}")

        if target_priority:
            lines.append(f"- **Target Priority:** {target_priority}")

        if recovery_reactions:
            lines.append("")
            lines.append("**Recovery Reactions:**")
            lines.extend(recovery_reactions)

        if unique_behavior:
            lines.append("")
            lines.append("**Unique Behavior:**")
            lines.append(f"- {unique_behavior}")

        if special_effect:
            lines.append("")
            lines.append("**Special Effect:**")
            lines.append(f"- {special_effect}")
...
with tabs[6]:
    ...
    # Site rule: difficulty is not selected here—Full Antagonist generates all four tiers.
    diff_choice = "Standard"
# --------------------------------------------------
    ...
    if st.button("ROLL FULL ANTAGONIST", key="btn_full_antagonist"):
        ...
        # ----- Core identity & stats (rolled ONCE for the site) -----
        ...
        # Roll role ONCE (Option A: same role for all difficulty tiers)
        enemy_role = roll_table("enemy_role", log=False, option="Standard")

        # Unique Trait ONCE (Option A: same trait for all difficulty tiers)
        # NOTE: this applies modifiers in the background; it is intentionally not persisted as a line item.
        _ = roll_table("unique_trait", log=False, option="Other")

        # Enemy Abilities ONCE (Option A: same ability list, scaled by difficulty)
        enemy_abilities_all = []
        if not st.session_state.get("suppress_enemy_ability", False):
            for _i in range(3):
                enemy_abilities_all.append(roll_table("enemy_ability", log=False))

        # Psychic ability ONLY if the rolled role is Psychic (role flag) — rolled once and reused
        psychic_ability = None
        role_mods = st.session_state.get("role_mods") or {}
        if bool(role_mods.get("use_psychic_ability_table")):
            psychic_ability = roll_table("psychic_ability", log=False)

        # Appearance & anatomy (rolled once)
        appearance_desc = build_appearance_description(limb_env_choice)

        difficulties = ["Easy", "Standard", "Elite", "Overwhelming"]
        ability_counts = {"Easy": 1, "Standard": 1, "Elite": 2, "Overwhelming": 3}

        tier_outputs = []

        for d in difficulties:
            # Stat block is the ONLY thing that changes by difficulty (plus how many abilities you reference)
            stat_block = roll_table("stat_block", log=False, option=d)

            n = min(ability_counts.get(d, 1), len(enemy_abilities_all))
            if n > 0:
                ability_lines = "\n".join(
                    [f"- **Enemy Ability {i}** — {enemy_abilities_all[i-1]}  " for i in range(1, n + 1)]
                )
            else:
                ability_lines = "- *(No Enemy Abilities)*  "

            psychic_line = f"- **Psychic Ability** — {psychic_ability}  " if psychic_ability else ""

            tier_md = f"""### {creature_name.upper()} — {d.upper()}
...
"""
            ...
            tier_md = tier_md.strip() + "\n"

            # Display each tier as its own clearly-labeled entry
            st.success(tier_md)

            # Persist each tier as a standalone block for quick encounter reference
            add_to_persistent(5, tier_md)

            tier_outputs.append(tier_md)

        # Log a single combined entry
        combined = "\n\n---\n\n".join(tier_outputs)
        add_to_log(combined)
# --------------------------------------------------
    ...
    with st.container(border=True):
        ...
        with threat_col:
                        # Pull Known Threat names directly from known_threat.csv so this dropdown stays in sync.
            threat_names = ["Spitter", "Clobber", "Taker", "Psybane", "Bomber", "Cecaelia"]
            try:
                _kdf = load_table_df("known_threat")
                if "name" in _kdf.columns:
                    # Preserve CSV order while removing duplicates
                    threat_names = list(dict.fromkeys(_kdf["name"].dropna().astype(str).tolist()))
            except Exception:
                pass
...
with tabs[9]:
    ...
    with col_info:
        ...
        if _wkey not in st.session_state:
            _seed = (initial_terrain or "Landing").strip() or "Landing"
            if terrain_options and _seed not in terrain_options:
                _seed = terrain_options[0]
            st.session_state[_wkey] = _seed
        ...
        if st.button("Roll Hex", key=f"btn_roll_hex_{selected_hex}"):
            ...
            st.success("Hex rolled (terrain + exploration) and added to notes.")
    ...
    with col_map:
        ...
        with st.expander("If clicking doesn’t select a hex, use this dropdown instead"):
            # Keep dropdown in sync with the currently selected hex.
            # NOTE: This MUST happen BEFORE the selectbox is instantiated on this run,
            # otherwise Streamlit will raise: "st.session_state.<key> cannot be modified after the widget ... is instantiated".
            if st.session_state.get("map_hex_dropdown_fallback") != st.session_state["selected_hex"]:

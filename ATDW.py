...
def _short_head(s: str) -> str:
    """Return the short label portion of a roll result."""
    if not s:
        return ""
    for sep in [":", "—", "–", "-"]:
        if sep in s:
...
def _exploration_short(exploration_result: str) -> str:
    """Abbreviate the planetside exploration outcome for hover tooltip."""
    t = (exploration_result or "").lower()
    if "hazard" in t:
        return "Hazard"
    if "findings" in t:
        return "Findings"
...
def ensure_map_state():
    """Normalize/repair the hex map state.

    Conventions:
      - hex['terrain'] stores the *standing-in* terrain for that hex (what you'd use as
        'previous_hex' when rolling the NEXT hex).
      - hex['terrain_set'] is True only when the hex has been rolled / explicitly set.
        This prevents unrolled hexes from accidentally inheriting the last terrain just
        because they were selected.

    This function also migrates older saves where rolled terrain was only stored in
    'td_short'.
    """
    ...
    # Back-compat + safety: infer terrain_set for rolled hexes.
    # (td_short and/or last/expl_short are written by the Roll Hex action.)
    for i, d in cleaned.items():
        td = (d.get("td_short") or "").strip()
        last = (d.get("last") or "").strip()
        exs = (d.get("expl_short") or "").strip()

        rolled_signal = bool(td or last or exs or d.get("terrain_set"))
        d["terrain_set"] = bool(rolled_signal)

        # Older saves: terrain for a rolled hex often lived only in td_short.
        # Keep 'terrain' aligned with the rolled terrain so the dropdown syncs cleanly.
        if td and (not (d.get("terrain") or "").strip() or (d.get("terrain") or "").strip().lower() != td.lower()):
            d["terrain"] = td

    st.session_state["hex_map"] = cleaned

    # Clamp selected hex so the app doesn't crash if it was out of range
    try:
    ...
    # Remember the last *standing-in* terrain (used as default for unrolled hexes)
...
def render_hex_plotly_map(hex_map: dict, selected_hex: int, zoom_level: float = 1.0, height_px: int | None = None):
    ...
    for n in render_order:
        ...
        td_short = (d.get("td_short") or "").strip()

        ex_short = (d.get("expl_short") or "").strip()

        parts = [p for p in [td_short, ex_short] if p]

        hover_lines.append("<br>".join(parts) if parts else f"Hex {n}")
...
with tabs[9]:
    ...
    # One-run flash messages (st.rerun clears st.success/st.error)
    if st.session_state.get("map_flash_success"):
        st.success(st.session_state["map_flash_success"])
        st.session_state["map_flash_success"] = None

    # Deferred widget sync helpers (avoid 'cannot be modified after widget...' errors)
    if "force_hex_dropdown_refresh_to" not in st.session_state:
        st.session_state["force_hex_dropdown_refresh_to"] = None
    if "map_prev_selected_hex" not in st.session_state:
        st.session_state["map_prev_selected_hex"] = None
    if "map_force_terrain_refresh_for" not in st.session_state:
        st.session_state["map_force_terrain_refresh_for"] = None
    if "map_force_terrain_value" not in st.session_state:
        st.session_state["map_force_terrain_value"] = None
    ...
    with col_info:
        ...
        # -----------------------
        # -----------------------
        # Terrain type selector + single Roll Hex button
        # -----------------------
        ...
        # Dropdown sync rules:
        #  - If this hex already has a rolled/explicit terrain (terrain_set=True), show that.
        #  - Otherwise, show the last rolled terrain (map_default_terrain), i.e. your "standing-in" terrain.
        has_stored_terrain = bool(d.get("terrain_set")) and bool((d.get("terrain") or "").strip())
        stored_terrain = (d.get("terrain") or "").strip() if has_stored_terrain else ""
        ...
        _wkey = f"map_terrain_{selected_hex}"
        selection_changed = (st.session_state.get("map_prev_selected_hex") != selected_hex)
        forced_refresh = (st.session_state.get("map_force_terrain_refresh_for") == selected_hex)

        # Only (re)seed when you select a different hex, or when we explicitly force a refresh after rolling/import.
        if selection_changed or forced_refresh:
            seed = (st.session_state.get("map_force_terrain_value") or initial_terrain).strip() if forced_refresh else initial_terrain

            # Normalize seed against dropdown options
            if terrain_options:
                if seed not in terrain_options:
                    match = next((o for o in terrain_options if str(o).strip().lower() == seed.lower()), None)
                    seed = match if match else terrain_options[0]

            st.session_state[_wkey] = seed

            # When selecting a rolled hex, treat it as your current standing-in terrain.
            if has_stored_terrain and stored_terrain:
                st.session_state["map_default_terrain"] = stored_terrain

            if forced_refresh:
                st.session_state["map_force_terrain_refresh_for"] = None
                st.session_state["map_force_terrain_value"] = None

            st.session_state["map_prev_selected_hex"] = selected_hex
        else:
            # First ever render of this particular widget key
            if _wkey not in st.session_state:
                seed = initial_terrain
                if terrain_options and seed not in terrain_options:
                    match = next((o for o in terrain_options if str(o).strip().lower() == seed.lower()), None)
                    seed = match if match else terrain_options[0]
                st.session_state[_wkey] = seed
        ...
        if st.button("Roll Hex", key=f"btn_roll_hex_{selected_hex}"):
            # -----------------------
            # 1) Terrain Difficulty
            # -----------------------
            td_result = roll_table("terrain_difficulty", option=terrain, log=False)
            td_head = _short_head(td_result)
            final_terrain = td_head
            ...
            # If Biome-Dependent, auto-roll the biome table using the selected biome
            if ("biome-dependent" in td_result.lower()) or ("biome dependent" in td_result.lower()) or (td_head.strip().lower() in {"biome-dependent", "biome dependent", "biomedependent"}):
                biome_choice = (biome or (st.session_state.get("map_default_biome") or "") or "Barren").strip()
                try:
                    bdf = load_table_df("biome_dependent_terrain")
                    row = bdf[bdf["biome"].astype(str).str.strip().str.lower() == biome_choice.lower()].sample(1).iloc[0]
                    bd_result = f"{row['result']}: {row['description']}"
                    add_to_persistent(4, f"Hex {selected_hex} — Biome-Dependent Terrain ({biome_choice}): {bd_result}")
                    add_to_log(f"Hex {selected_hex} — Biome-Dependent Terrain ({biome_choice}): {bd_result}")
                    append_to_hex_notes(selected_hex, f"Biome-Dependent Terrain ({biome_choice}): {bd_result}")
                    final_terrain = str(row.get("result", td_head)).strip() or td_head
                except Exception as e:
                    add_to_log(f"Hex {selected_hex} — [ERROR] Biome-Dependent Terrain roll failed: {e}")

            # Save short terrain for hover + update default terrain for next hex
            hex_map[selected_hex]["td_short"] = final_terrain or td_head
            if final_terrain:
                st.session_state["map_default_terrain"] = final_terrain
                # Persist rolled terrain as this hex's standing-in terrain
                hex_map[selected_hex]["terrain"] = final_terrain
                hex_map[selected_hex]["terrain_set"] = True

            # -----------------------
            # 2) Planetside Exploration (from this hex)
            # -----------------------
            biome_choice = (biome or (st.session_state.get("map_default_biome") or "") or "Barren").strip()
            exploration_result = roll_table("planetside_exploration", log=False)

            hex_map[selected_hex]["expl_short"] = _exploration_short(exploration_result)
            ...
            if "findings" in exploration_result.lower():
                ...
                # Mark a site in the underlying data immediately.
                # We don't force a rerun here (rerun was causing the map to 'explode');
                # instead we force the saved value to True during the sync step below.
                site_force_true = True
            ...
            # Auto-append FULL results block into Notes BEFORE the notes widget is created
            append_to_hex_notes(selected_hex, "\n".join(note_lines))

            st.session_state["hex_map"] = hex_map

            # Bump Plotly UI revision to prevent map 'explosion' after heavy actions.
            st.session_state["map_uirev"] = st.session_state.get("map_uirev", 0) + 1

            # Force the terrain dropdown to reflect the newly rolled terrain on the NEXT rerun
            st.session_state["map_force_terrain_refresh_for"] = selected_hex
            st.session_state["map_force_terrain_value"] = (final_terrain or td_head or st.session_state.get("map_default_terrain", "Landing"))

            # Flash message (since we're about to rerun)
            st.session_state["map_flash_success"] = "Hex rolled (terrain + exploration) and added to notes."
            st.rerun()
        ...
        # Use update() so we do NOT clobber keys written by other actions (e.g., "last" from exploration).

        # IMPORTANT: Do not write a terrain onto an unrolled hex just because it was selected.
        # If terrain_set is False, the dropdown is showing the *standing-in* (previous) terrain for the NEXT roll,
        # not the destination hex's terrain.
        terrain_to_store = terrain if bool(hex_map[selected_hex].get("terrain_set")) else (hex_map[selected_hex].get("terrain") or "")

        hex_map[selected_hex].update({
            "visited": visited,
            "party": party_here,
            "site": (site_present or site_force_true),
            "special": special_flag,
            "name": name,
            "biome": biome,
            "terrain": terrain_to_store,
            "notes": notes,
        })
        st.session_state["hex_map"] = hex_map
        ...
        with cB:
            if st.button("Add Hex Summary to Log", key=f"btn_log_hex_{selected_hex}"):
                add_to_log(
                    ...
                    f"Biome: {biome or '(unset)'} | Terrain: {terrain_to_store or '(unset)'} | Visited: {visited} | "
    ...
    with col_map:
        ...
        # Apply any deferred sync to the fallback dropdown BEFORE it's created this run.
        if st.session_state.get("force_hex_dropdown_refresh_to") is not None:
            st.session_state["map_hex_dropdown_fallback"] = int(st.session_state["force_hex_dropdown_refresh_to"])
            st.session_state["force_hex_dropdown_refresh_to"] = None
            ...
            try:
                ...
                for k, v in loaded.items():
                    ...
                    if 1 <= kk <= MAP_HEX_COUNT and isinstance(v, dict):
                        ...
                        v.setdefault("terrain_set", False)
                        v.setdefault("td_short", "")
                        v.setdefault("expl_short", "")
                        v.setdefault("notes", "")
                        v.setdefault("last", "")
                        # Infer terrain_set for older saves
                        if not v.get("terrain_set") and ((v.get("td_short") or "").strip() or (v.get("expl_short") or "").strip() or (v.get("last") or "").strip()):
                            v["terrain_set"] = True
                        # If td_short exists, keep terrain aligned (rolled terrain)
                        if (v.get("td_short") or "").strip():
                            v["terrain"] = (v.get("td_short") or v.get("terrain") or "").strip()
                        cleaned[kk] = v
                ...
                # Defer dropdown + terrain sync to the next rerun (avoids Streamlit widget-state errors)
                st.session_state["force_hex_dropdown_refresh_to"] = st.session_state["selected_hex"]
                st.session_state["map_prev_selected_hex"] = None
                st.session_state["map_force_terrain_refresh_for"] = st.session_state["selected_hex"]
                try:
                    st.session_state["map_force_terrain_value"] = (cleaned.get(int(st.session_state["selected_hex"]), {}) or {}).get("terrain") or st.session_state.get("map_default_terrain", "Landing")
                except Exception:
                    st.session_state["map_force_terrain_value"] = st.session_state.get("map_default_terrain", "Landing")
                st.session_state["map_flash_success"] = "Map imported."
                st.rerun()

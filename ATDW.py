...
with tabs[9]:
    ...
    # --- Apply pending Map JSON import BEFORE any Map widgets are instantiated ---
    # This prevents Streamlit errors like:
    # 'st.session_state.<key> cannot be modified after the widget with key <key> is instantiated.'
    if "_pending_hex_map_import" in st.session_state:
        st.session_state["hex_map"] = st.session_state.pop("_pending_hex_map_import")

        # Validate selection
        sel = int(st.session_state.get("selected_hex", 1) or 1)
        if sel not in st.session_state["hex_map"]:
            sel = 1
        st.session_state["selected_hex"] = sel

        # Keep fallback dropdown in sync (safe here because the widget hasn't been created yet in this run)
        st.session_state["map_hex_dropdown_fallback"] = sel

        # Clear per-hex widget keys so imported values appear correctly
        for k in list(st.session_state.keys()):
            if re.match(r"^map_(v|party|site|special|name|biome|terrain|biome_dep|notes)_\d+$", str(k)):
                st.session_state.pop(k, None)

        # Nudge Plotly re-render without altering user zoom/height sliders
        st.session_state["map_uirev"] = int(st.session_state.get("map_uirev", 0) or 0) + 1
    ...
    with col_info:
        ...
        if st.button("ROLL FULL EXPLORATION (this hex)", key=f"btn_hex_explore_{selected_hex}"):
            ...
            st.success("Exploration rolled and added to notes.")
    ...
    with col_map:
        ...
        if up is not None:
            ...
            # Guard: file_uploader persists across reruns; only import when the bytes actually change.
            if st.session_state.get("_last_map_import_digest") != digest:
                st.session_state["_last_map_import_digest"] = digest
                ...
                try:
                    ...
                    # Stage the import so we can apply it BEFORE widgets (prevents key mutation errors).
                    st.session_state["_pending_hex_map_import"] = cleaned
                    st.rerun()

...
with tabs[9]:
    ...
    with col_info:
        ...
        # Apply any pending terrain dropdown updates (safe; avoids Plotly map 'explosion')
        pending_updates = st.session_state.pop("map_pending_terrain_updates", None)
        if isinstance(pending_updates, dict) and pending_updates:
            for _k, _v in pending_updates.items():
        ...
        # If you click a new (unrolled) hex, auto-sync this dropdown to the last rolled terrain
        # (so players don't have to track it manually). A hex counts as 'rolled' once it has
        # terrain/exploration short fields or a last exploration result.
        _rolled = bool((d.get("td_short") or "").strip() or (d.get("expl_short") or "").strip() or (d.get("last") or "").strip())
        if st.session_state.get("map_last_selected_hex") != selected_hex:
            st.session_state["map_last_selected_hex"] = selected_hex
            if not _rolled:
                _desired = (st.session_state.get("map_default_terrain") or "Landing").strip() or "Landing"
                _wkey = f"map_terrain_{selected_hex}"
                if _wkey in st.session_state and _desired in terrain_options:
                    st.session_state[_wkey] = _desired
        ...
        if st.button("Roll Hex", key=f"btn_roll_hex_{selected_hex}"):
            ...
            # Persist the rolled terrain on this hex and queue a safe UI sync for the dropdown
            if final_terrain and final_terrain in terrain_options:
                hex_map[selected_hex]["terrain"] = final_terrain
                st.session_state.setdefault("map_pending_terrain_updates", {})
                st.session_state["map_pending_terrain_updates"][f"map_terrain_{selected_hex}"] = final_terrain
        ...
        rolled_this_hex = bool((hex_map[selected_hex].get("td_short") or "").strip() or (hex_map[selected_hex].get("expl_short") or "").strip() or (hex_map[selected_hex].get("last") or "").strip())
        hex_map[selected_hex].update({
            ...
            "name": name,
            "biome": biome,
            "terrain": (terrain if rolled_this_hex else ""),
            "notes": notes,
        })
        st.session_state["hex_map"] = hex_map

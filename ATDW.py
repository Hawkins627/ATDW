...
with tabs[9]:
    ...
    with col_info:
        ...
        # Apply a pending terrain update (from the last roll) BEFORE rendering the selectbox.
        _pending_hex = st.session_state.get("map_pending_terrain_hex")
        _pending_val = st.session_state.get("map_pending_terrain_value")
        if _pending_hex == selected_hex and _pending_val:
            _match = next((o for o in terrain_options if o.lower() == str(_pending_val).lower()), None)
            if _match:
                st.session_state[_wkey] = _match
            st.session_state["map_pending_terrain_hex"] = None
            st.session_state["map_pending_terrain_value"] = None
        ...
        if st.button("Roll Hex", key=f"btn_roll_hex_{selected_hex}"):
            ...
            # Update the default terrain (and the dropdown itself) to the last terrain TYPE rolled.
            _new_default = None
            if final_terrain and any(o.lower() == str(final_terrain).lower() for o in terrain_options):
                _new_default = final_terrain
            elif td_head and any(o.lower() == str(td_head).lower() for o in terrain_options):
                _new_default = td_head

            if _new_default:
                st.session_state["map_default_terrain"] = _new_default
                st.session_state["map_pending_terrain_hex"] = selected_hex
                st.session_state["map_pending_terrain_value"] = _new_default
            ...
            st.session_state["map_roll_success_msg"] = "Hex rolled (terrain + exploration) and added to notes."
            st.rerun()
        _msg = st.session_state.pop("map_roll_success_msg", None)
        if _msg:
            st.success(_msg)

    ...
    with col_info:
        ...
        with st.container(border=True):
            ...
            if "Landing" in terrain_options:
                terrain_options = ["Landing"] + [t for t in terrain_options if t != "Landing"]

            # If this hex doesn't have a terrain yet, default to the last terrain you picked/rolled.
            stored_terrain = (d.get("terrain", "") or "").strip()
            default_terrain = (st.session_state.get("map_default_terrain", "") or "").strip()
            initial_terrain = stored_terrain or default_terrain or "Landing"

            # If the Roll Hex button decided a new terrain, we stage it and apply it *before* the widget is created.
            # (Streamlit does not allow setting a widget\'s session_state key after the widget has been instantiated.)
            pending = st.session_state.pop("map_pending_terrain_update", None)
            if isinstance(pending, dict):
                try:
                    _hx = int(pending.get("hex"))
                    _terr = pending.get("terrain")
                    if _terr:
                        st.session_state[f"map_terrain_{_hx}"] = _terr
                        st.session_state["map_default_terrain"] = _terr
                        if _hx == selected_hex:
                            initial_terrain = _terr
                except Exception:
                    pass
        ...
        # Show the last roll output for this hex (survives reruns)
        roll_display_key = f"map_roll_hex_display_{selected_hex}"
        if st.session_state.get(roll_display_key):
            with st.expander("Last roll results (this hex)", expanded=True):
                for msg in st.session_state[roll_display_key]:
                    kind = (msg.get("kind") or "info").lower()
        ...
        st.markdown("### Roll Hex")
        with st.container(border=True):
            st.caption("Rolls Terrain Difficulty → (auto) Biome-Dependent Terrain if needed → Full Planetside Exploration.")

            if st.button("🎲 Roll Hex", key=f"btn_map_roll_hex_{selected_hex}"):

                msgs = []
                prev_terrain = terrain or "Landing"
                ...
                try:
                    ...
                    if "previous_hex" in td_df.columns:
                        sub = td_df[td_df["previous_hex"].astype(str).str.lower() == str(prev_terrain).lower()]
                    else:
                        sub = td_df
                    ...
                    # Prefer result/description, then title/description, else generic formatter
                    if "result" in row and "description" in row and pd.notna(row["result"]) and pd.notna(row["description"]):
                        td_text = f"{row['result']}: {row['description']}"
                    elif "title" in row and "description" in row:
                        title = str(row.get("title", "")).strip()
                        desc = str(row.get("description", "")).strip()
                        td_text = f"{title}: {desc}" if title and desc else (title or desc)
                    else:
                        td_text = format_row_for_display("terrain_difficulty", row)

                    # Guess the "next" terrain type (what we should set the dropdown to)
                    for col in ("result", "title", "next_hex", "next", "new_hex", "terrain", "next_terrain"):
                        if col in row and pd.notna(row[col]) and str(row[col]).strip():
                            next_terrain = str(row[col]).strip()
                            break

                    if next_terrain and ":" in next_terrain:
                        next_terrain = next_terrain.split(":", 1)[0].strip()

                    if not next_terrain:
                        guess = re.split(r"[:–-]", str(td_text))[0].strip()
                        next_terrain = guess if guess else None

                except Exception as e:
                    td_text = f"[ERROR] Terrain difficulty roll failed: {e}"
                    next_terrain = None

                add_to_persistent(4, f"Hex {selected_hex} — Terrain ({prev_terrain}): {td_text}")
                add_to_log(f"Hex {selected_hex} — Terrain ({prev_terrain}): {td_text}")
                append_to_hex_notes(selected_hex, f"Terrain ({prev_terrain}): {td_text}")
                msgs.append({"kind": "success", "text": f"Terrain Difficulty ({prev_terrain}) → {td_text}"})

                # Normalize next terrain to an exact dropdown option (case-insensitive match)
                final_terrain = None
                if next_terrain:
                    for opt in terrain_options:
                        if str(opt).strip().lower() == str(next_terrain).strip().lower():
                            final_terrain = opt
                            break

                # -----------------------
                # 2) Biome-Dependent Terrain (AUTO, only if needed)
                # -----------------------
                if final_terrain and final_terrain.strip().lower() == "biome-dependent":
                    biome_choice = (biome or st.session_state.get("map_default_biome") or "Barren").strip() or "Barren"

                    bd_text = ""
                    bd_next = None

                    try:
                        bd_df = load_table_df("biome_dependent_terrain")

                        if "biome" in bd_df.columns:
                            sub = bd_df[bd_df["biome"].astype(str).str.lower() == biome_choice.lower()]
                        else:
                            sub = bd_df
                        ...
                        if "result" in row and "description" in row:
                            bd_text = f"{row['result']}: {row['description']}"
                        else:
                            bd_text = format_row_for_display("biome_dependent_terrain", row)

                        if "result" in row and pd.notna(row["result"]) and str(row["result"]).strip():
                            bd_next = str(row["result"]).strip()

                    except Exception as e:
                        bd_text = f"[ERROR] Biome-dependent terrain roll failed: {e}"
                        bd_next = None

                    add_to_persistent(4, f"Hex {selected_hex} — Biome-Dependent Terrain ({biome_choice}): {bd_text}")
                    add_to_log(f"Hex {selected_hex} — Biome-Dependent Terrain ({biome_choice}): {bd_text}")
                    append_to_hex_notes(selected_hex, f"Biome-Dependent Terrain ({biome_choice}): {bd_text}")
                    msgs.append({"kind": "success", "text": f"Biome-Dependent Terrain ({biome_choice}) → {bd_text}"})

                    # If the biome-dependent result is itself a terrain option, use it as the new default
                    if bd_next:
                        for opt in terrain_options:
                            if str(opt).strip().lower() == bd_next.strip().lower():
                                final_terrain = opt
                                break

                # Update terrain dropdown for THIS hex + the default for the NEXT hex
                if final_terrain:
                    st.session_state["map_default_terrain"] = final_terrain
                    st.session_state["map_pending_terrain_update"] = {"hex": selected_hex, "terrain": final_terrain}

                # -----------------------
                # 3) Full Planetside Exploration (existing behavior)
                # -----------------------
                ...
                msgs.append({"kind": "success", "text": f"Planetside Exploration → {exploration_result}"})
                ...
                if "findings" in exploration_result.lower():
                    ...
                    msgs.append({"kind": "info", "text": f"Findings → {findings_result}"})
                    ...
                    msgs.append({"kind": "warning", "text": f"Hazard ({biome_choice}) → {hazard_result}"})
                    ...
                    msgs.append({"kind": "warning", "text": "Site found! Roll full site in the Mission tab."})

                elif "nothing" in exploration_result.lower():
                    add_to_persistent(4, f"Hex {selected_hex} — Exploration: Nothing found.")
                    note_lines.append("Nothing found.")
                    msgs.append({"kind": "info", "text": "Nothing found."})

                else:
                    note_lines.append("Result not recognized (check CSV formatting).")
                    msgs.append({"kind": "warning", "text": "Exploration result not recognized (check CSV formatting)."})
                ...
                # Store the UI output + rerun so the terrain dropdown reflects the new rolled terrain immediately
                st.session_state[roll_display_key] = msgs
                st.rerun()
